...
<template>
  <div class="settingsPage">
    ...
    <div class="sCard card">
      ...
      <div class="sStatusHint" v-else-if="modelSettings.configured">
        钥匙已保存：{{ modelSettings.maskedApiKey }}
      </div>
      <div class="sStatusHint" v-else>
        还没配置 DeepSeek 钥匙，铃湾暂时不能帮你处理事情。
      </div>
    </div>
    ...
    <div class="sCard card">
      ...
      <button class="primary" style="margin-top: 10px;" @click="$emit('go', 'deepseek-config')">
        前往配置
      </button>
    </div>
    ...
    <div class="sCard card">
      ...
      <button class="ghost" style="margin-top: 8px;" @click="$emit('go', 'advanced')">
        进入高级设置 →
      </button>
    </div>
  </div>
