  ...
  <div class="obs-form-overlay" @click.self="$emit('cancel')">
    <div class="obs-form">
      ...
      <label class="obs-form-field">
        <span>日期</span>
        <input v-model="form.date" type="date" class="obs-form-input" :disabled="saving" />
      </label>
      <label class="obs-form-field">
        <span>类型</span>
        <select v-model="form.type" class="obs-form-input" :disabled="saving">
          <option v-for="t in TYPES" :key="t.value" :value="t.value">{{ t.label }}</option>
        </select>
      </label>
      <label class="obs-form-field">
        <span>标题</span>
        <input v-model="form.title" type="text" class="obs-form-input" :disabled="saving" />
      </label>
      <label class="obs-form-field">
        <span>内容</span>
        <textarea v-model="form.content" class="obs-form-input obs-form-textarea" rows="5" :disabled="saving" />
      </label>
      <div v-if="saveError" class="obs-form-error">{{ saveError }}</div>
