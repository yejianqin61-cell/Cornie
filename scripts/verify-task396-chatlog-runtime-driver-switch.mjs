...
async function run() {
  ...
  try {
    ...
    assert(defaultRepository.driver === CHATLOG_REPOSITORY_DRIVERS.betterSqlite3, '默认 chatlog repository 应切到 better-sqlite3')

    const chatlog = createChatlogService(harness.store)
    const dayRecord = chatlog.getByDate('2026-06-30', { limit: 10, cursor: 0 })
    assert(dayRecord.storage.driver === CHATLOG_REPOSITORY_DRIVERS.betterSqlite3, '默认 chatlog service 应通过 better-sqlite3 提供服务')
    ...
    assert(searchResult.storage.driver === CHATLOG_REPOSITORY_DRIVERS.betterSqlite3, '消息片段检索也应走 better-sqlite3')
    ...
    assert(forcedSqlJsDayRecord.storage.driver === CHATLOG_REPOSITORY_DRIVERS.sqljs, '显式指定 sql.js 时应仍可回退')
