...
export function createChatlogRepository(store, { driver } = {}) {
  // 469（方案 B）：聊天记录统一由 sql.js 单引擎提供读写，
  // 不再打开磁盘文件做 better-sqlite3 只读查询，消除"双份真相"与原生模块 ABI 风险。
  const requestedDriver = normalizeString(driver)
  return createSqlJsChatlogRepository(store, {
    requestedDriver: requestedDriver || 'sqljs',
    fallbackReason: 'single_engine_sqljs_469'
  })
}
